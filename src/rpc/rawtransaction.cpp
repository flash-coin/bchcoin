...
void TxToJSON(const Config &config, const CTransaction &tx,
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        UniValue out(UniValue::VOBJ);
        out.push_back(Pair("value", ValueFromAmount(txout.nValue)));
        out.push_back(Pair("valueSat", txout.nValue.GetSatoshis())); // WL add for BC API
        out.push_back(Pair("n", (int64_t)i));
        UniValue o(UniValue::VOBJ);
        ScriptPubKeyToJSON(config, txout.scriptPubKey, o, true);
        out.push_back(Pair("scriptPubKey", o));
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            if (chainActive.Contains(pindex)) {
			    entry.push_back(Pair("height", pindex->nHeight));// WL add for BC API
                entry.push_back(
                    Pair("confirmations",
                ...
                entry.push_back(Pair("height", -1)); // WL add for BC API
                entry.push_back(Pair("confirmations", 0));
            }
        }
    }
...
static UniValue getrawtransaction(const Config &config,
    ...
    // LOCK(cs_main); // WL add for BC API
    ...
    TxToJSONExpanded(config, *tx, hashBlock, result, nHeight, nConfirmations, nBlockTime); // WL add for BC API
    return result;
}
